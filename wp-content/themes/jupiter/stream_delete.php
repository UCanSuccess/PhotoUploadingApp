...
global $wpdb;
	...
	for ($i=0; $i < count($streams_arr); $i++) { 
		...
		$query = $wpdb->prepare("DELETE FROM wp_streams WHERE id='".$streams_arr[$i]."'",'');
		$user = $wpdb->get_results($query);	
		$query = $wpdb->prepare("DELETE FROM wp_photos WHERE stream_id='".$streams_arr[$i]."'",'');
		$user = $wpdb->get_results($query);	
		$query = $wpdb->prepare("DELETE FROM wp_stream_group WHERE stream_id='".$streams_arr[$i]."'",'');
		$user = $wpdb->get_results($query);	
	}
	// exit;
